...
export default async function NotificationsPage() {
  ...
  const [recipients, branches, recentLogs, channels, telegramCfg, smtpCfg, visionDetectionRaw, cacheRaw, throttleRaw, metricsRows, topCachedImages] =
    await Promise.all([
      ...
      prisma.visionCache.findMany({
        orderBy: { hits: 'desc' },
        take: 5,
        include: { /* no relation needed */ },
      }).catch(() => [] as Array<{ imageHash: string; campaignId: number; hits: number; lastHitAt: Date | null }>),
    ]);
  ...
  const cacheHitRate = totalEvals > 0 ? ((stats.last7d.cacheHit / totalEvals) * 100).toFixed(1) : '0.0';
  // gpt-4o approx cost: $2.50/1M input + $10/1M output. 1 vision call ~ 1500 input + 300 output tokens ≈ $0.007
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Bot tracking cache hits / API calls / throttled submissions từ DB. Update realtime sau mỗi event.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="rounded-md border p-3">
              ...
              <div className="text-2xl font-bold tabular-nums text-green-600">{stats.today.cacheHit}</div>
            </div>
            <div className="rounded-md border p-3">
              <div className="text-xs text-muted-foreground">API calls (today)</div>
              <div className="text-2xl font-bold tabular-nums">{stats.today.cacheMiss}</div>
            </div>
            <div className="rounded-md border p-3">
              <div className="text-xs text-muted-foreground">Throttled (today)</div>
              <div className="text-2xl font-bold tabular-nums text-orange-600">{stats.today.throttled}</div>
            </div>
            <div className="rounded-md border p-3">
              <div className="text-xs text-muted-foreground">Cost saved (7d)</div>
              <div className="text-2xl font-bold tabular-nums text-green-600">${costSavedUsd}</div>
            </div>
          </div>
          ...
          <div className="text-sm space-y-1.5 border-t pt-3">
            <div className="flex justify-between">
              ...
              <span className="font-mono font-medium">{cacheHitRate}% ({stats.last7d.cacheHit} / {totalEvals} evals)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Throttled (7d):</span>
              <span className="font-mono font-medium">{stats.last7d.throttled} duplicate submissions blocked</span>
            </div>
          ...
          {topCachedImages.length > 0 && (
            <div className="border-t pt-3">
              <div className="text-xs font-medium text-muted-foreground mb-2">Top cached images (most reused)</div>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {topCachedImages.map((c) => (
                    <TableRow key={c.imageHash + c.campaignId}>
                      <TableCell className="font-mono text-xs">{c.imageHash.slice(0, 12)}…</TableCell>
                      <TableCell className="text-center text-xs">#{c.campaignId}</TableCell>
                      <TableCell className="text-right font-mono text-xs">{c.hits}</TableCell>
